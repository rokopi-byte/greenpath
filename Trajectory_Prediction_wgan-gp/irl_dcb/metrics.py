...
def compute_mm(human_trajs, model_trajs, im_w, im_h, tasks=None):
    ...
    for traj in model_trajs:
        ...
        all_mm_scores.append((task,
                              np.mean([
                                  multimatch(traj, gt_traj, (im_w[traj['name']], im_h[traj['name']]))[:4]
...
def create_distance(normal):
    return lambda x, y: euclidean(x, y) / normal
...
def averageLCS(human, generated, p_size):
    mean = np.array([])
    counts = np.array([])
    for traj in generated:
        img_name = traj['name']
        task = traj['task']
        points = np.array([[traj['X'][i] // p_size[img_name][0],
                            traj['Y'][i] // p_size[img_name][1]]
                           for i in range(len(traj['X']))], dtype=float)
        gt_trajs = list(
            filter(lambda x: x['name'] == img_name and x['task'] == task,
                   human))
        value = np.array([])
        ...
        if len(gt_trajs) == 0:
            continue
        for human_traj in gt_trajs:
            points_h = np.array([[human_traj['X'][i] // p_size[img_name][0],
                                  human_traj['Y'][i] // p_size[img_name][1]] for i in range(len(human_traj['X']))],
                                dtype=float)
            index = -1
            distance = 3
            for i in range(len(points_h)):
                dst = euclidean(points_h[i], points[0])
                if dst < distance:
                    index = i
                    distance = dst
            if index == -1:
                continue
            points_h = points_h[index:]
            if len(points_h) < 5:
                continue
            length = LCS(points, points_h)
            if length > 1:
                value = np.append(value, [length/len(points_h)])
                if length >= len(points_h)/2:
                    count += 1
        if len(value) > 0:
            mean = np.append(mean, [np.mean(value)])
            counts = np.append(counts, [count/len(gt_trajs)])

        if len(gt_trajs) == 0:
            continue
    if len(mean) > 0:
        return np.mean(mean), np.mean(counts)
    else:
        return 0.0, 0


def dtw(human, generated, w, h):
    mean = np.array([])
    for traj in generated:
        img_name = traj['name']
        task = traj['task']
        points = np.array([[traj['X'][i], traj['Y'][i]] for i in range(len(traj['X']))], dtype=float)
        gt_trajs = list(
            filter(lambda x: x['name'] == img_name and x['task'] == task,
                   human))
        value = np.array([])
        normalization_value = 2*np.sqrt((w[img_name]**2)+(h[img_name]**2))
        if len(gt_trajs) == 0:
            continue
        for human_traj in gt_trajs:
            points_h = np.array([[human_traj['X'][i], human_traj['Y'][i]] for i in range(len(human_traj['X']))], dtype=float)
            index = -1
            distance = 50
            for i in range(len(points_h)):
                dst = euclidean(points_h[i], points[0])
            ...
            if len(points_h) < 5:
                continue
            distance, path = fastdtw(points, points_h, dist=create_distance(normalization_value))
            value = np.append(value, [distance])
        if len(value) > 0:
            mean = np.append(mean, [np.mean(value)])
